...
function loadCodeSyntaxHighlighterAssets()
{
    ...
    $context['html_headers'] .= '
        <link href="' . $settings['default_theme_url'] . '/css/CodeSyntaxHighlighter/' . 'shCore.css" rel="stylesheet" type="text/css" />
        <link href="' . $settings['default_theme_url'] . '/css/CodeSyntaxHighlighter/' . $modSettings['code_syntax_highlighter_theme'] . '.css" rel="stylesheet" type="text/css" />
        ';
    ...
    foreach ($codes as $codeId => $code) {
            ...
            $codes[$codeId] = array(
                ...
                'validate' => create_function('&$tag, &$data, $disabled', '
                    if (!isset($disabled[\'code\']))
					    $data[0] = parse_bbc($data[0], false, \'' . $cache_id . '\');
            	'),
                'block_level' => true,
        ...
        elseif ($code['tag'] == 'code' && $code['type'] == 'unparsed_content') {
            ...
            $codes[$codeId] = array(
                ...
                'validate' => create_function('&$tag, &$data, $disabled', '
                    if (!isset($disabled[\'code\']))
					    $data = parse_bbc($data, false, \'' . $cache_id . '\');
            	'),
                'block_level' => true,
                'disabled_content' => '<pre>$1</pre>',
            );
